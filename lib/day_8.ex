...
"""
  ...
  def call(input \\ @_input) do
    ...
    find_largest(:ets.first(:registry), 0)
  end
  ...
  defp lexer(token) do
    [reg, comp] = String.split(token, "if")

    if comp_hold?(comp) do
      do_comp(reg)
    end
  end

  defp do_comp(comp) do
    [reg, action, number] = String.split(comp, " ", trim: true)

    n_value = lookup(reg)
    number = String.to_integer(number)

    case action do
      "inc" -> :ets.insert(:registry, {reg, n_value + number})
      "dec" -> :ets.insert(:registry, {reg, n_value - number})
      _ -> "Nothing for #{action}"
    end
  end

  defp comp_hold?(comp) do
    [reg, operator, number] = String.split(comp, " ", trim: true)
  ...
  defp lookup(reg) do
    case :ets.lookup(:registry, reg) do
      [] -> (
      :ets.insert(:registry, {reg, 0})
      0
      )
      [{_, value}] -> value
    end
  ...
  defp find_largest(:"$end_of_table", n) do
    n
  end
  defp find_largest(reg, n) do
    [{_, value}] = :ets.lookup(:registry, reg)
    new_reg = :ets.next(:registry, reg)
    if value >= n, do: find_largest(new_reg, value), else: find_largest(new_reg, n)
  end
end
